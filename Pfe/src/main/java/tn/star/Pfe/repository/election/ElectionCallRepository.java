...
public interface ElectionCallRepository extends JpaRepository<ElectionCall, Long> {
    Page<ElectionCall> findByStatus(CallStatus status, Pageable pageable);

    Optional<ElectionCall> findByIdAndStatus(Long id, CallStatus status);

    Page<ElectionCall> findAllByStatusOrderByCreatedAtDesc(CallStatus status, Pageable pageable);
    ...
    Page<ElectionCall> findOpenCallsBeforeDeadline(CallStatus status, LocalDateTime now, Pageable pageable);
    ...
    Optional<ElectionCall> findFirstByStatusOrderByCreatedAtDesc(CallStatus status);
}